...
def calculate_middle(coordinates):
    if not coordinates:
        return [(0, 0)]  # Return default coordinates if list is empty
...
def calculate_zoom_level(coordinates):
    if not coordinates:
        return 1  # Return minimum zoom if no coordinates
...
async def get_reviews_screenshot(coords, id):
    if not coords:
        return None, None  # Return None if no coordinates are provided
