...
def search_xakepok(target_username):
    print("--search_xakepok")
    members_df = pl.read_parquet(xakepok_members)
    posts_df = pl.read_parquet(xakepok_posts)
    all_result = ""
    members_filtered_df = members_df.filter(members_df[:, 4] == target_username)
...
def search_opensc_ws(target_username):
...
def search_carderpro(target_username):
...
def search_inattack_part2(target_username):
    ...
    all_result = ""
    members_filtered_df = members_df.filter(pl.col('members_display_name') == target_username)
    if members_filtered_df.shape[0] > 0:
...
def search_inattack_part1(target_username):
...
def search_inattack_user(target_username):
...
def search_zloy_db(target_username):
    print("--search_zloy_db")
    df = pl.read_parquet(zloy_path)
    filtered_df = df.filter(pl.col('username') == target_username)
