...
from utils.ghunt_email.base import *
from utils.ghunt_email.utils import *
from utils.ghunt_email.knowledge import get_gmaps_type_translation
from utils.ghunt_email.utils import TMPrinter , ppnb
...
def get_reviews(as_client: httpx.Client, gaia_id: str) -> Tuple[str, Dict[str, int], List[MapsReview], List[MapsPhoto]]:
    ...
    req =  as_client.get(f"https://www.google.com/locationhistory/preview/mas?authuser=0&hl=en&gl=us&pb={config.templates['gmaps_pb']['stats'].format(gaia_id)}")
    ...
    with alive_bar(total_reviews, receipt=False) as bar:
        for category in ["reviews", "photos"]:
            ...
            while True:
                if first:
                    req =  as_client.get(f"https://www.google.com/locationhistory/preview/mas?authuser=0&hl=en&gl=us&pb={config.templates['gmaps_pb'][category]['first'].format(gaia_id)}")
                    first = False
                else:
                    req =  as_client.get(f"https://www.google.com/locationhistory/preview/mas?authuser=0&hl=en&gl=us&pb={config.templates['gmaps_pb'][category]['page'].format(gaia_id, next_page_token)}")
                data = json.loads(req.text[5:])
...
def output(err: str, stats: Dict[str, int], reviews: List[MapsReview], photos: List[MapsPhoto], gaia_id: str):
    ...
    map_result += f"[+] Average rating : {ppnb(avg_ratings)}/5\n"
    ...
    if types_and_tags:
        ...
        for type, type_count in types.items():
            ...
            print(f"\n🏨 {translation if translation else type.title()} [{type_count}]")
            map_result += f"\n🏨 {translation if translation else type.title()} [{type_count}]\n"
    ...
    confidence, locations = calculate_probable_location(geolocator, reviews_and_photos, config.gmaps_radius)
